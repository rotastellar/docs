...
console.log(`ISS: ${iss.position?.lat}, ${iss.position?.lon}`);
```
...
for await (const sat of client.listSatellitesPaginated({
  constellation: 'Starlink'
})) {
