...
Handle large result sets with the `PaginatedResponse` type:

```rust
use rotastellar::types::PaginatedResponse;
...
println!("Page has {} items, has_more={}", page.items.len(), page.has_more);
...
// Stream-based iteration (auto-fetches pages)
use futures::StreamExt;

let mut stream = client.list_satellites()
    .constellation("Starlink")
    .stream();

while let Some(sat) = stream.next().await {
    let sat = sat?;
    println!("{}", sat.name);
}
```
...
|-------|------|-------------|
| `items` | `Vec<T>` | Current page of items |
| `has_more` | `bool` | Whether more pages exist |
| `total` | `Option<usize>` | Total count (if available) |
| `limit` | `usize` | Page size |
| `offset` | `usize` | Current offset |
